...
const Layout: React.FC<LayoutProps> = ({ children }) => {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
            <AppBar position="static" elevation={2} sx={{ bgcolor: '#fff', color: 'primary.main', borderRadius: 0, boxShadow: '0 2px 12px 0 rgba(0,0,0,0.06)' }}>
                <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', px: { xs: 1, sm: 3 } }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, minWidth: 0 }}>
                        <Box component={Link} to="/" sx={{ display: 'flex', alignItems: 'center', textDecoration: 'none', minWidth: 0 }}>
                            <img src="/assets/logo.png" alt="Logo" style={{ width: 36, height: 36, borderRadius: '50%', marginRight: 10 }} />
                            <Typography
                                variant="h6"
                                sx={{ fontWeight: 700, color: '#000000', letterSpacing: 1, fontSize: { xs: 18, sm: 22 }, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
                            >
                                FIT 9to5
                            </Typography>
                        </Box>
